...
from lib.base import Message, Conversation
...
class quarks(commands.Cog):
    ...
    async def on_ready(self):
        logger.info(f"We have logged in as {self.bot.user}. Invite URL: {BOT_INVITE_URL}")
        completion.MY_BOT_NAME = self.bot.user.name
        completion.MY_BOT_EXAMPLE_CONVOS = []
        # We still need this to run once to have the DEFAULT_CONVO in case we dont have any in the DB
        for c in EXAMPLE_CONVOS:
            messages = []
            for m in c.messages:
                if m.user == "Lenard":
                    messages.append(Message(user=self.bot.user.name, text=m.text))
                else:
                    messages.append(m)
    ...
    async def chat_command(self, int: discord.Interaction, message: str) -> None:
        try:
